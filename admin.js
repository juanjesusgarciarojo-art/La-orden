...
function escucharEstadoMision() {
    ...
    onSnapshot(groupRef, (snapshot) => {
        ...
        // 1. Mostrar estado de la clave grupal
        if (data.tiempo_inicio) {
            if (data.pausado) {
                ...
                // Iniciar sincronización de contador
            ...
            // También sincronizar cita aunque no haya empezado la misión
            const inputDia = document.getElementById('cfg-cita-dia');
            const inputHora = document.getElementById('cfg-cita-hora');
            if (inputDia && document.activeElement !== inputDia) inputDia.value = data.cita_dia || "";
            if (inputHora && document.activeElement !== inputHora) inputHora.value = data.cita_hora || "";
        }
    });
}
    ...
    if (btnTogglePause) {
        btnTogglePause.addEventListener('click', async () => {
            ...
            if (!isPausado) {
                ...
                // REANUDAR: Ajustar tiempo_inicio sumando la duración de la pausa
                const now = Date.now();
                const pausaInicio = data.pausa_inicio.toDate().getTime();
    ...
    if (btnFullReset) {
        btnFullReset.addEventListener('click', async () => {
            ...
            try {
                ...
                // 3. Resetear grupo (borrar tiempos y marcar activo)
                const groupRef = doc(db, "grupos", GROUP_ID);
                await updateDoc(groupRef, {
                    tiempo_inicio: null,
                    tiempo_fin: null,
